...
dirro='/data/obs/obs/gridded/terraclim/MAT/2C/';
...
for yyr=1:length(yy)
 yr=yy(yyr);
 load([dirr,'vap1_2c_',num2str(yr+1960)]);%vapdata=vap;
 load([dirr,'tmax1_2c_',num2str(yr+1960)]);%tmaxdata=data;
 load([dirr,'tmin1_2c_',num2str(yr+1960)]);%tmindata=data;clear data
f=find(tmaxdata<tmindata);tmaxdata(f)=tmindata(f);
 ...
 load([dirr,'srad1_2c_',num2str(yr+1960)]);
 load([dirr,'ws1_2c_',num2str(yr+1960)]);
 tmaxdata=shiftdim(tmaxdata,2);
 tmindata=shiftdim(tmindata,2);
 winddata=shiftdim(wsdata,2);clear ws
 sraddata=shiftdim(sraddata,2);clear srad
...
petdata=single(petdata);
 save([dirr,'pet1_',num2str(yr+1960)],'-v7.3','petdata');
 clear *data ET
 yr
end
